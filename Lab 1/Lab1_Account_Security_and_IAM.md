...
| Permissions are granted via groups/roles, not directly to individual users. | Complete | Administrative access is correctly inherited through `Admins`, and Kubernetes uses a Role plus RoleBinding. However, the lab explicitly attaches `AmazonS3ReadOnlyAccess` directly to `Analyst_hakim`, so this statement is not wholly true for the IAM configuration. In production, a dedicated analyst group would better satisfy this practice. |
